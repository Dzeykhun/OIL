  ...
  if (AppDM.Cars.Active) and (not AppDM.Cars.IsEmpty) then
    try
      ...
      begin
        ...
        InsuranсeNumEdit.Text := AppDM.CarsDetail.FieldByName('INS_NUM').AsString;
...
begin
  ...
  try
    ...
    on Err : Exception do
      ShowMessage('Ошибка получении параметров машины.' + #13 + ' Сообщения ' + err.Message);
  end;
end;
